...
function App() {
  ...
  function validation(e: React.SyntheticEvent) {
    ...
    if (urlInput === "") {
      ...
      setValidationMsg(1);
      return;
    } else if (bookmarks.find((o: Bookmark) => o.url === urlInput)) {
      // duplicate url
      e.preventDefault();
      setValidationMsg(2);
      return;
  ...
  function createBookmark() {
    const urlInput = (document.getElementById("url-field") as HTMLInputElement)
      .value;
    const notesInput = (
      document.getElementById("notes-field") as HTMLInputElement
    ).value;
  ...
  function editBookmark(e: React.SyntheticEvent, id: any) {
    ...
    setBookmarks((prevBookmarks: []) =>
      prevBookmarks
        ...
        .sort(function (a, b) {
          var x = a["lastEditedRaw"];
          var y = b["lastEditedRaw"];
  ...
  const currentBookmarks = bookmarks.slice(
    ...
    indexOfLastBookmark
  ); // shows only bookmarks between first and last indexed bookmark on page X
